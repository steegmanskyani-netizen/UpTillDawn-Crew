...
export default function OperationsClient(p:Props){
 ...
 <button disabled={busy} className="w-full rounded-xl border p-4 font-bold" onClick={()=>run(()=>p.activeBreak?work('stop_break',{break_id:p.activeBreak.id}):work('start_break',{session_id:p.activeSession!.id}))}>{p.activeBreak?'STOP BREAK':'START BREAK'}</button>
 <button disabled={busy} className="w-full rounded-xl bg-red-700 p-4 font-bold text-white" onClick={()=>run(()=>work('stop_work',{session_id:p.activeSession!.id}))}>STOP WORK</button>
 <button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(async()=>{const {error}=await s.rpc('upt_request_check_out',{p_event:p.activeSession!.event_id});if(error)throw error;setMsg('Check-out aangevraagd. STOP WORK blijft een aparte actie.')})}>CHECK-OUT AANVRAGEN</button>
 </section>}
 <section className="space-y-3"><h2 className="text-xl font-bold">Mijn shifts</h2>{!p.shifts.length&&<p>Er zijn geen shifts toegewezen.</p>}{p.shifts.map(shift=>{const checkin=p.checkins.find(c=>c.user_id===p.userId&&c.event_id===shift.event_id&&c.workplace_id===shift.workplace_id);return <article key={shift.id} className="space-y-3 rounded-2xl border p-4"><h3 className="font-bold">{p.events.find(e=>e.id===shift.event_id)?.name} · {p.workplaces.find(w=>w.id===shift.workplace_id)?.name}</h3><p>{new Date(shift.scheduled_start).toLocaleString('nl-BE')} → {new Date(shift.scheduled_end).toLocaleString('nl-BE')}</p><p>Check-in: {checkin?.status||'nog niet aangevraagd'}</p>
 {(!checkin||checkin.status==='rejected')&&<button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(()=>requestCheckin(shift))}>CHECK-IN AANVRAGEN</button>}
 {checkin?.status==='approved'&&!p.activeSession&&<button disabled={busy} className="w-full rounded-xl bg-violet-600 p-4 font-bold" onClick={()=>run(()=>work('start_work',{event_id:shift.event_id,shift_id:shift.id}))}>START WORK</button>}
 {checkin?.status==='approved'&&!p.activeSession&&<button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(async()=>{const {error}=await s.rpc('upt_request_check_out',{p_event:shift.event_id});if(error)throw error;setMsg('Check-out aangevraagd.')})}>CHECK-OUT AANVRAGEN</button>}
 {p.activeSession?.event_id===shift.event_id&&p.activeSession.shift_id!==shift.id&&<button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(()=>work('transition',{session_id:p.activeSession!.id,workplace_id:shift.workplace_id}))}>NEW WORKPLACE — CONFIRM TRANSITION</button>}
 </article>})}</section>
 {p.manager&&<section className="space-y-3 rounded-2xl border p-4"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold">Live crew-status</h2><span className="text-sm text-muted-foreground">{p.liveSessions.length} actief</span></div>{!p.liveSessions.length&&<p className="text-muted-foreground">Momenteel is er geen zichtbare crew aan het werk.</p>}{p.liveSessions.map(ws=>{const crew=p.crewDirectory.find(m=>m.id===ws.user_id);const shift=p.liveShifts.find(x=>x.id===ws.shift_id);const workplace=p.workplaces.find(w=>w.id===shift?.workplace_id);const onBreak=p.liveBreaks.some(b=>b.work_session_id===ws.id&&!b.ended_at);return <article key={ws.id} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{crew?.full_name||'Crewlid'}</p><p className="text-sm text-muted-foreground">{workplace?.name||'Werkplek'} · gestart {new Date(ws.started_at).toLocaleTimeString('nl-BE',{hour:'2-digit',minute:'2-digit'})}</p>{crew?.phone_number&&<a className="text-sm underline" href={`tel:${crew.phone_number}`}>{crew.phone_number}</a>}</div><span className={`rounded-full px-3 py-1 text-xs font-bold ${onBreak?'bg-amber-500/20 text-amber-300':'bg-emerald-500/20 text-emerald-300'}`}>{onBreak?'PAUZE':'AAN HET WERK'}</span></div></article>})}</section>}
 {p.manager&&<section className="space-y-3"><h2 className="text-xl font-bold">Goedkeuringen</h2>{[...p.checkins.filter(c=>c.status==='pending').map(c=>({...c,kind:'in' as const})),...p.checkouts.filter(c=>c.status==='pending').map(c=>({...c,kind:'out' as const}))].map(c=><article key={c.id} className="rounded-xl border p-4"><p>Check-{c.kind} · {p.workplaces.find(w=>w.id===c.workplace_id)?.name} · {c.user_id}</p>{c.kind==='in'&&c.selfie_path&&<button className="underline" onClick={()=>run(async()=>{const {data,error}=await s.storage.from('checkin-selfies').createSignedUrl(c.selfie_path!,60);if(error)throw error;window.open(data.signedUrl,'_blank','noopener,noreferrer')})}>Selfie bekijken</button>}<div className="mt-3 flex gap-3"><button disabled={busy} className="rounded-lg bg-violet-600 p-3" onClick={()=>run(()=>decide(c.kind,c.id,true))}>Goedkeuren</button><button disabled={busy} className="rounded-lg border p-3" onClick={()=>run(()=>decide(c.kind,c.id,false))}>Afwijzen</button></div></article>)}</section>}
 </main>
}
