...
CREATE TABLE IF NOT EXISTS briefings (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,workplace_id UUID REFERENCES workplaces(id) ON DELETE CASCADE,title TEXT NOT NULL,body TEXT NOT NULL,version INTEGER NOT NULL DEFAULT 1,required BOOLEAN NOT NULL DEFAULT true,created_by UUID REFERENCES user_profiles(id),created_at TIMESTAMPTZ NOT NULL DEFAULT now(),updated_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS briefing_acknowledgements (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),briefing_id UUID NOT NULL REFERENCES briefings(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,version INTEGER NOT NULL,acknowledged_at TIMESTAMPTZ NOT NULL DEFAULT now(),UNIQUE(briefing_id,user_id,version));
CREATE TABLE IF NOT EXISTS tasks (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,workplace_id UUID REFERENCES workplaces(id) ON DELETE CASCADE,title TEXT NOT NULL,description TEXT,status task_status NOT NULL DEFAULT 'not_started',assigned_user_id UUID REFERENCES user_profiles(id) ON DELETE SET NULL,created_by UUID REFERENCES user_profiles(id),completed_by UUID REFERENCES user_profiles(id),completed_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT now(),updated_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS check_ins (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,workplace_id UUID REFERENCES workplaces(id) ON DELETE SET NULL,status approval_status NOT NULL DEFAULT 'pending',gps_status gps_status NOT NULL DEFAULT 'not_checked',latitude NUMERIC(9,6),longitude NUMERIC(9,6),accuracy_m NUMERIC,distance_m NUMERIC,remote BOOLEAN NOT NULL DEFAULT false,selfie_path TEXT,requested_at TIMESTAMPTZ NOT NULL DEFAULT now(),decided_at TIMESTAMPTZ,decided_by UUID REFERENCES user_profiles(id),notes TEXT);
CREATE TABLE IF NOT EXISTS check_outs (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,status approval_status NOT NULL DEFAULT 'pending',requested_at TIMESTAMPTZ NOT NULL DEFAULT now(),decided_at TIMESTAMPTZ,decided_by UUID REFERENCES user_profiles(id),notes TEXT);
CREATE TABLE IF NOT EXISTS work_sessions (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,shift_id UUID REFERENCES shifts(id) ON DELETE SET NULL,started_at TIMESTAMPTZ NOT NULL DEFAULT now(),ended_at TIMESTAMPTZ,status work_status NOT NULL DEFAULT 'active',start_gps_status gps_status NOT NULL DEFAULT 'not_checked',stop_gps_status gps_status NOT NULL DEFAULT 'not_checked',created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE UNIQUE INDEX IF NOT EXISTS one_active_work_session ON work_sessions(user_id) WHERE ended_at IS NULL;
CREATE TABLE IF NOT EXISTS break_sessions (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),work_session_id UUID NOT NULL REFERENCES work_sessions(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,started_at TIMESTAMPTZ NOT NULL DEFAULT now(),ended_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE UNIQUE INDEX IF NOT EXISTS one_active_break ON break_sessions(user_id) WHERE ended_at IS NULL;
CREATE TABLE IF NOT EXISTS workplace_transitions (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),work_session_id UUID NOT NULL REFERENCES work_sessions(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,from_workplace_id UUID REFERENCES workplaces(id),to_workplace_id UUID NOT NULL REFERENCES workplaces(id),transitioned_at TIMESTAMPTZ NOT NULL DEFAULT now(),confirmed_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS chat_channels (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),kind TEXT NOT NULL CHECK(kind IN ('organization','event','workplace','private')),event_id UUID REFERENCES events(id) ON DELETE CASCADE,workplace_id UUID REFERENCES workplaces(id) ON DELETE CASCADE,name TEXT,created_at TIMESTAMPTZ NOT NULL DEFAULT now());
...
CREATE TABLE IF NOT EXISTS messages (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),channel_id UUID NOT NULL REFERENCES chat_channels(id) ON DELETE CASCADE,sender_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE RESTRICT,body TEXT NOT NULL CHECK(length(trim(body))>0),moderated_at TIMESTAMPTZ,moderated_by UUID REFERENCES user_profiles(id),created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS message_attachments (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),message_id UUID NOT NULL REFERENCES messages(id) ON DELETE CASCADE,storage_path TEXT NOT NULL,mime_type TEXT,created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS crew_notifications (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),user_id UUID NOT NULL REFERENCES user_profiles(id) ON DELETE CASCADE,title TEXT NOT NULL,body TEXT,kind TEXT NOT NULL DEFAULT 'info',read_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS upt_audit_logs (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),actor_id UUID REFERENCES user_profiles(id) ON DELETE SET NULL,action TEXT NOT NULL,entity_type TEXT NOT NULL,entity_id UUID,metadata JSONB NOT NULL DEFAULT '{}'::jsonb,created_at TIMESTAMPTZ NOT NULL DEFAULT now());
...
CREATE TABLE IF NOT EXISTS event_templates (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),name TEXT NOT NULL,configuration JSONB NOT NULL DEFAULT '{}'::jsonb,created_by UUID REFERENCES user_profiles(id),created_at TIMESTAMPTZ NOT NULL DEFAULT now());
