...
export function ChatClient({
  ...
  const current=cache[effectiveSelected]||{messages:[],attachments:{}}
    ...
    async function load(){
      ...
      if(ids.length){
        ...
        for(const item of signed){
          if(!item)continue
          ;(grouped[item.messageId]||=[]).push({url:item.url,mimeType:item.mimeType})
        }
      }
      if(!alive)return
      setCache(previous=>({...previous,[effectiveSelected]:{messages:ordered,attachments:grouped}}))
    }
  ...
  async function sendMessage(){
    ...
    try{
      if(file){
        ...
        setBody('');setFile(null);setFileKey(key=>key+1)
        setStatus(navigator.onLine
          ? 'Media en bericht zijn bewaard voor serververwerking.'
          : 'Media en bericht zijn lokaal bewaard en worden verzonden zodra je online bent.')
      }else{
  ...
  if(!selectedChannel){
    return <section className="rounded-2xl border p-6"><h1 className="text-2xl font-black">Chat</h1><p className="mt-2 text-muted-foreground">Er zijn momenteel geen beschikbare chats.</p></section>
  }
  ...
  return <section className="relative flex min-h-[calc(100dvh-9rem)] flex-col overflow-hidden bg-background md:min-h-[70vh] md:rounded-3xl md:border">
    <header className="sticky top-0 z-30 border-b bg-background/95 px-3 py-3 backdrop-blur md:px-4">
      <button type="button" onClick={()=>setPickerOpen(open=>!open)} className="flex w-full items-center gap-3 rounded-2xl p-1 text-left hover:bg-muted/60" aria-expanded={pickerOpen}>
        <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border bg-muted"><Users className="h-5 w-5"/></div>
      ...
      {pickerOpen&&<div className="absolute left-3 right-3 top-[4.5rem] z-40 max-h-[65vh] overflow-y-auto rounded-2xl border bg-card p-2 shadow-2xl md:left-4 md:right-4">
        ...
        {!!organizationChannels.length&&<div className="mt-2">
          <p className="px-2 py-1 text-[11px] font-bold uppercase tracking-[.16em] text-muted-foreground">Algemeen</p>
          {organizationChannels.map(channel=><ChannelButton key={channel.id} channel={channel} selected={effectiveSelected} onChoose={chooseChannel} name={channelName(channel)}/>)}
        </div>}
    ...
    <div className="flex-1 space-y-3 overflow-y-auto px-3 py-4 md:px-5">
      ...
      {current.messages.map(message=>{
        ...
        const initials=(mine?(directory.get(userId)?.full_name||'Jij'):senderName).split(/\s+/).map(part=>part[0]).join('').slice(0,2).toUpperCase()
        ...
        return <article key={message.id} className={`flex items-end gap-2 ${mine?'justify-end':'justify-start'}`}>
          ...
          <div className={`max-w-[82%] ${mine?'items-end':'items-start'} flex flex-col`}>
            ...
            <div className={`rounded-2xl px-3.5 py-2.5 shadow-sm ${mine?'rounded-br-md bg-violet-600 text-white':'rounded-bl-md border bg-card'}`}>
              ...
              {!moderated&&current.attachments[message.id]?.map((attachment,index)=>
    ...
    <div className="sticky bottom-0 z-20 border-t bg-background/95 p-3 backdrop-blur">
      {file&&<div className="mb-2 flex items-center justify-between rounded-xl border bg-card px-3 py-2 text-xs">
        ...
        <button type="button" onClick={()=>{setFile(null);setFileKey(key=>key+1)}} className="ml-3 rounded-full p-1 hover:bg-muted" aria-label="Media verwijderen"><X className="h-4 w-4"/></button>
      </div>}
      <form className="flex items-end gap-2" onSubmit={event=>{event.preventDefault();void sendMessage()}}>
        <label className="flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-full border bg-card hover:bg-muted" aria-label="Media toevoegen">
          <ImagePlus className="h-5 w-5"/>
          <input key={fileKey} type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" onChange={event=>setFile(event.target.files?.[0]||null)} className="sr-only"/>
        </label>
        <textarea
          aria-label="Bericht"
          maxLength={4000}
        ...
        />
        <button type="submit" disabled={busy||!effectiveSelected||(!body.trim()&&!file)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-violet-600 text-white disabled:opacity-40" aria-label="Versturen">
...
function ChannelButton({channel,selected,onChoose,name}:{channel:Tables<'chat_channels'>;selected:string;onChoose:(id:string)=>void;name:string}){
  return <button type="button" onClick={()=>onChoose(channel.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left ${channel.id===selected?'bg-violet-600 text-white':'hover:bg-muted'}`}>
    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-current/15 bg-black/10"><Users className="h-4 w-4"/></div>
