...
export default function OperationsClient(p:Props){
 ...
 {p.personalWork&&<section className="space-y-3"><h2 className="text-xl font-bold">Mijn diensten</h2>{!p.shifts.length&&<p>Er zijn geen diensten toegewezen.</p>}{p.shifts.map(shift=>{const checkin=p.checkins.find(c=>c.user_id===p.userId&&c.event_id===shift.event_id&&c.workplace_id===shift.workplace_id);return <article key={shift.id} className="space-y-3 rounded-2xl border p-4"><h3 className="font-bold">{p.events.find(e=>e.id===shift.event_id)?.name} · {p.workplaces.find(w=>w.id===shift.workplace_id)?.name}</h3><p>{new Date(shift.scheduled_start).toLocaleString('nl-BE')} → {new Date(shift.scheduled_end).toLocaleString('nl-BE')}</p><p>{p.manager&&!p.isAdmin?'Starturen':'Inklokken'}: {checkin ? nlStatus(checkin.status) : 'nog niet aangevraagd'}</p>
 {(!checkin||checkin.status==='rejected')&&<button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(()=>requestCheckin(shift))}>{p.manager&&!p.isAdmin?'UREN STARTEN AANVRAGEN':'INKLOKKEN AANVRAGEN'}</button>}
 {checkin?.status==='pending'&&p.manager&&!p.isAdmin&&<p className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-sm">Wacht op goedkeuring door een beheerder. Na goedkeuring start de urenteller automatisch vanaf de aanvraagtijd.</p>}
 {checkin?.status==='approved'&&!p.activeSession&&!(p.manager&&!p.isAdmin)&&<button disabled={busy} className="w-full rounded-xl bg-violet-600 p-4 font-bold" onClick={()=>run(()=>work('start_work',{event_id:shift.event_id,shift_id:shift.id}))}>WERK STARTEN</button>}
