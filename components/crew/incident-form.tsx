...
export function IncidentForm({
  ...
  return <form className="space-y-3 rounded-2xl border border-red-500 p-5" onSubmit={async e=>{
    ...
    try{
      ...
      if(file){
        ...
        setStatus(navigator.onLine
          ? 'URGENT melding en media zijn bewaard voor serververwerking.'
          : 'URGENT melding en media zijn lokaal bewaard en worden automatisch verzonden zodra je online bent.')
      }else{
    ...
    <select name="event" required value={eventId} onChange={e=>{
      const next=e.target.value
      setEventId(next)
      if(!contexts.some(context=>context.event_id===next&&context.workplace_id===workplaceId))setWorkplaceId('')
    }} className="w-full rounded-lg border bg-background p-3">
      <option value="">Selecteer evenement</option>
      {events.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}
    </select>
    <select name="workplace" value={workplaceId} onChange={e=>setWorkplaceId(e.target.value)} className="w-full rounded-lg border bg-background p-3">
