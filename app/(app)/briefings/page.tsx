...
export default async function Page(){
 ...
 return <main className="space-y-5 p-4 md:p-8"><h1 className="text-3xl font-black">Instructies</h1>
 {user.isAdmin&&<div className="grid gap-4 lg:grid-cols-2">
  <form action={createBriefing} className="grid gap-3 rounded-xl border p-4"><h2 className="font-bold">Nieuwe algemene instructie</h2><select name="event_id" required className="border bg-background p-3">{events?.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select><input name="title" required placeholder="Titel" className="border bg-background p-3"/><textarea name="body" required placeholder="Algemene instructie" className="min-h-28 border bg-background p-3"/><button className="rounded-xl bg-violet-600 p-3">Instructie aanmaken</button></form>
