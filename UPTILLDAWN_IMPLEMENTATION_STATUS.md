...
3. **Full offline-first shell:** authenticated operational screens still cannot be treated as a complete reopen-offline application after the browser/app is fully closed. Incident/chat photo queues are implemented, but offline continuation of every workflow and offline profile/check-in image handling are not complete.
