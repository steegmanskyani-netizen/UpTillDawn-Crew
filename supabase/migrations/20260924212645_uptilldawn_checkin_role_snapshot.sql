...
alter table public.check_ins
  drop constraint if exists check_ins_requested_role_check;

alter table public.check_ins
  add constraint check_ins_requested_role_check
  check (requested_role is null or requested_role in ('staff','responsible_lead','admin'));
...
begin
  ...
  insert into public.check_ins(
    user_id,event_id,workplace_id,type,status,selfie_path,selfie_url,gps_status,
    latitude,longitude,accuracy_m,remote,requested_at,requested_role,created_at,
    approved_by,approved_at,decided_by,decided_at
  )
  values(
    auth.uid(),p_event,p_workplace,'check-in','pending',p_selfie_path,null,
    coalesce(nullif(trim(p_gps_status),''),'not_checked'),
    p_latitude,p_longitude,p_accuracy_m,p_remote,now(),v_role,now(),
    null,null,null,null
  )
...
begin
  ...
  v_requester_role := coalesce(v_check_in.requested_role,public.upt_effective_role(v_check_in.user_id));
  ...
  if p_approve and v_requester_role='responsible_lead' then
    ...
    values(
      ...
      jsonb_build_object(
        ...
        'requested_role',v_requester_role,
