...
export default async function Page(){
 ...
 const visibleBriefs=isAdmin?(briefs||[]):isResponsible?(briefs||[]).filter(b=>assignedEventIds.has(b.event_id)&&readableEventIds.has(b.event_id)):(briefs||[]).filter(staffCanReadBriefing)
 const visiblePersonal=isAdmin?(personal||[]):isResponsible?(personal||[]).filter(i=>assignedEventIds.has(i.event_id)&&readableEventIds.has(i.event_id)):(personal||[]).filter(staffCanReadInstruction),hasStaffInstruction=visibleBriefs.length>0||visiblePersonal.length>0
 ...
 else if(isResponsible){const {data:responsibleMemberships}=await s.from('event_members').select('event_id').eq('user_id',user.id).in('event_role',['responsible_lead','admin']);const eventIds=[...new Set((responsibleMemberships||[]).map(row=>row.event_id))];if(eventIds.length){const [{data:eventRows},{data:workplaceRows}]=await Promise.all([s.from('events').select('id,name').in('id',eventIds).neq('status','archived').gte('end_at','now').order('start_at'),s.from('workplaces').select('id,name,event_id').in('event_id',eventIds).eq('is_active',true).order('sort_order')]);events=eventRows||[];workplaces=workplaceRows||[];const directories=await Promise.all(eventIds.map(async eventId=>{const {data}=await s.rpc('upt_responsible_event_members',{p_event:eventId,p_workplace:null as unknown as string});return{eventId,people:data||[]}}));const uniquePeople=new Map<string,AssignmentPerson>();for(const directory of directories)for(const person of directory.people){uniquePeople.set(person.id,{id:person.id,full_name:person.full_name});memberships.push({event_id:directory.eventId,workplace_id:null,user_id:person.id})}people=[...uniquePeople.values()].sort((a,b)=>(a.full_name||'').localeCompare(b.full_name||'','nl'))}}
