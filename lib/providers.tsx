...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{
    ...
    const refreshAuth=async()=>{
      const [{data:{user:currentUser}},{data:{session:currentSession}}]=await Promise.all([supabase.auth.getUser(),supabase.auth.getSession()])
      if(!alive)return
    ...
    void refreshAuth()
    const timer=window.setInterval(()=>{if(user)void loadProfile(user.id)},30000)
    const focus=()=>{if(user)void loadProfile(user.id)}
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,currentSession)=>{
      ...
      else{setProfile(null);setRoles([]);setRoleModeState(null);setIsOwner(false);void clearOfflineIdentity().catch(()=>{})}
    })
  ...
  return <AuthContext.Provider value={{user,session,profile,roles:effectiveRoles,isAdmin:effectiveIsAdmin,realIsAdmin:Boolean(realIsAdmin),isOwner,roleMode,setRoleMode,loading,refreshProfile:async()=>{if(user)await loadProfile(user.id)}}}>{children}</AuthContext.Provider>
}
