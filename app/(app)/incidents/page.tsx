  ...
  return <main className="mx-auto max-w-4xl space-y-5 p-4 pb-28 md:p-8">
    ...
    {error ? <p>Meldingen konden niet worden geladen.</p> : data?.map(i =>
      <article key={i.id} className="rounded-xl border p-4">
        ...
        {i.photo_path && <IncidentPhoto path={i.photo_path}/>}
...
function IncidentPhoto({ path }: { path: string }) {
  return <span className="mt-2 block text-sm text-muted-foreground">Foto gekoppeld aan incident</span>
}
