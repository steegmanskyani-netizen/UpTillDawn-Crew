...
export default function OperationsClient(p:Props){
 ...
 <section className="rounded-2xl border p-4"><label className="flex gap-3"><input type="checkbox" checked={remote} onChange={e=>setRemote(e.target.checked)}/>Inklokken op afstand met nieuwe werkplekselfie</label>{remote&&<input className="mt-3" type="file" accept="image/jpeg,image/png,image/webp" capture="user" onChange={e=>setFile(e.target.files?.[0]||null)}/>}</section>
 ...
 <button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(async()=>{const {error}=await s.rpc('upt_request_check_out',{p_event:p.activeSession!.event_id});if(error)throw error;setMsg('Check-out aangevraagd. WERK STOPPEN blijft een aparte actie.')})}>UITKLOKKEN AANVRAGEN</button>
 </section>}
 <section className="space-y-3"><h2 className="text-xl font-bold">Mijn diensten</h2>{!p.shifts.length&&<p>Er zijn geen diensten toegewezen.</p>}{p.shifts.map(shift=>{const checkin=p.checkins.find(c=>c.user_id===p.userId&&c.event_id===shift.event_id&&c.workplace_id===shift.workplace_id);return <article key={shift.id} className="space-y-3 rounded-2xl border p-4"><h3 className="font-bold">{p.events.find(e=>e.id===shift.event_id)?.name} · {p.workplaces.find(w=>w.id===shift.workplace_id)?.name}</h3><p>{new Date(shift.scheduled_start).toLocaleString('nl-BE')} → {new Date(shift.scheduled_end).toLocaleString('nl-BE')}</p><p>Inklokken: {checkin?.status||'nog niet aangevraagd'}</p>
 ...
 {checkin?.status==='approved'&&!p.activeSession&&<button disabled={busy} className="w-full rounded-xl border p-4" onClick={()=>run(async()=>{const {error}=await s.rpc('upt_request_check_out',{p_event:shift.event_id});if(error)throw error;setMsg('Check-out aangevraagd.')})}>UITKLOKKEN AANVRAGEN</button>}
