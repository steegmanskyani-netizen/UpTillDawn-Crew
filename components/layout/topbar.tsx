...
export function Topbar(){
 ...
 return <>
  {isAdmin&&testMode&&<div className="flex flex-wrap items-center justify-center gap-3 bg-amber-500 px-3 py-2 text-sm font-bold text-black">
    ...
    <select aria-label="Testrol" value={testRole || "employee"} onChange={e=>setTestRole(e.target.value as "employee"|"responsible_lead")} className="rounded-lg border border-black/30 bg-white px-3 py-1 text-black">
      <option value="employee">Personeel</option>
      <option value="responsible_lead">Verantwoordelijke</option>
    </select>
    <button onClick={()=>setTestMode(false)} className="rounded-lg bg-black px-3 py-1 text-white">TESTMODUS DEACTIVEREN</button>
  </div>}
  <header className="flex min-h-16 items-center justify-between gap-3 border-b bg-card px-4">
   ...
   <div className="flex items-center gap-3 text-sm">
    ...
    {isAdmin&&!testMode&&<button type="button" onClick={()=>setTestMode(true)} className="rounded-lg border border-amber-500/50 px-3 py-2 font-bold">TESTMODUS ACTIVEREN</button>}
