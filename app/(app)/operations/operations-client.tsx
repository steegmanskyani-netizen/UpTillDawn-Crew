...
export default function OperationsClient(p:Props){
 ...
 const {error}=await s.rpc('upt_request_check_in',{p_event:shift.event_id,p_workplace:shift.workplace_id,p_remote:remote,...(path?{p_selfie_path:path}:{})});if(error)throw error;setFile(null);setMsg(p.manager&&!p.isAdmin?'Inklokken aangevraagd. Een beheerder moet dit goedkeuren voordat je de uren kunt starten.':'Inklokken aangevraagd. Goedkeuring start de werktimer niet.')
 }
 ...
 {checkin?.status==='pending'&&p.manager&&!p.isAdmin&&<p className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-sm">Wacht op goedkeuring door een beheerder voordat je de uren kunt starten.</p>}
